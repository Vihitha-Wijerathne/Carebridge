...
class MedAdaptor (private val projectList: ArrayList<ProjectModel>) : RecyclerView.Adapter<MedAdaptor.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.project_list_view, parent,false)
        return ViewHolder(itemView, mListner)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val currentItem = projectList[position]
        if(currentItem.projecType=="Medicine") {
